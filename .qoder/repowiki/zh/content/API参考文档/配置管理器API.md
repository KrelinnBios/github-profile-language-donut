...
## 更新摘要
**所做更改**   
- 更新了配置文件编码处理部分，新增对UTF-8-SIG编码的支持说明
- 增强了故障排查指南中关于BOM标记问题的诊断和修复建议
- 完善了配置文件读取流程的编码处理细节
...
**更新** 配置文件现在支持UTF-8-SIG编码格式，能够正确处理包含BOM（字节顺序标记）的配置文件。这确保了来自不同编辑器和操作系统的配置文件都能被正确解析。
...
- **编码问题**：如果遇到配置文件开头出现乱码或解析错误，可能是由于BOM（字节顺序标记）导致的。配置文件现在支持UTF-8-SIG编码，如果仍遇到问题，请尝试重新保存配置文件为UTF-8无BOM格式。
...
**新增** UTF-8-SIG编码支持：
- 自动检测：配置加载器现在能够自动识别和处理UTF-8-SIG编码的配置文件
- BOM处理：内置的BOM标记处理逻辑会正确跳过字节顺序标记，确保配置内容正常解析
- 兼容性：支持Windows编辑器（如记事本）默认创建的UTF-8带BOM配置文件
