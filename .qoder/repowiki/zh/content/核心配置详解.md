...
## 更新摘要
**变更内容**
- 更新了配置文件编码支持章节，详细说明UTF-8-SIG编码的增强功能
- 新增跨平台兼容性说明，特别是Windows系统BOM标记处理
- 更新了配置加载流程中的编码处理细节
...
**更新** 配置文件编码支持已增强，现在使用UTF-8-SIG编码替代标准UTF-8，自动处理Windows系统生成的配置文件中的BOM标记，提高跨平台兼容性
...
### 配置文件编码与跨平台兼容性
**新增** 配置文件编码支持是确保跨平台兼容性的关键特性。系统现已全面升级编码处理机制：
...
**更新** 新增了编码相关的故障排查指导，特别关注Windows系统下的BOM标记问题
...
## 结论
通过统一的配置模型与清晰的模块边界，用户可以灵活定制图表的外观与行为。遵循本文的字段规范、示例与最佳实践，可在保证可读性的前提下获得稳定的输出效果。结合环境变量与继承机制，可实现跨环境一致的配置管理与安全的敏感信息注入。**最新的UTF-8-SIG编码支持确保了跨平台兼容性，特别是在Windows环境下处理BOM标记的能力，大大提升了用户体验和部署便利性。**

## 附录
- 术语
  - 甜甜圈图：中心空白的环形统计图，常用于展示比例分布
  - 对比度策略：根据背景与前景自动或手动调整明暗以提升可读性
  - **BOM标记**：字节顺序标记（Byte Order Mark），用于标识文本文件的编码类型
  - **UTF-8-SIG**：带签名的UTF-8编码，包含BOM标记以提高跨平台兼容性
- 参考
