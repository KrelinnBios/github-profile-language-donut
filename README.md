...
<p align="center">
  <a href="https://github.com/KrelinnBios/github-profile-language-donut/actions/workflows/test.yml"><img src="https://img.shields.io/github/actions/workflow/status/KrelinnBios/github-profile-language-donut/test.yml?branch=main&style=flat-square&label=%E6%B5%8B%E8%AF%95&color=247344" alt="测试状态"></a>
...
- 收到类型为 `code-pushed` 的 `repository_dispatch` 事件。

Action 只在 SVG 内容、README 引用或旧图文件发生变化时输出 `changed=true`，因此相同数据不会产生重复提交。

### 在代码提交后更新

如果希望代码仓库有新提交时立即更新主页，可将 [`examples/notify-profile.yml`](./examples/notify-profile.yml) 复制到每个代码仓库：

1. 创建一个只授权个人主页仓库的 fine-grained personal access token。
2. 为该 Token 授予个人主页仓库的 **Contents: Read and write** 权限。
3. 在代码仓库的 **Settings → Secrets and variables → Actions** 中添加 `PROFILE_REPO_TOKEN`。
4. 将示例中的 `YOUR_GITHUB_USERNAME` 替换为自己的用户名。

通知工作流通过 `paths-ignore` 忽略 Markdown 和许可证文件，因此仅修改这些文档不会触发主页更新。若默认分支不是 `main`，请同步修改工作流中的分支名。

请不要把 Token 直接写入仓库文件或日志。
...
主页工作流中的 `contents: write` 用于提交生成的 SVG 和 README；Action 本身不会向其他仓库写入内容。跨仓库通知所用的 `PROFILE_REPO_TOKEN` 应只授权个人主页仓库，并使用满足需求的最小权限。
...
检查代码仓库是否存在 `PROFILE_REPO_TOKEN`、Token 是否能访问个人主页仓库且拥有 `Contents: Read and write`、事件名是否为 `code-pushed`，以及主页工作流是否位于默认分支。
